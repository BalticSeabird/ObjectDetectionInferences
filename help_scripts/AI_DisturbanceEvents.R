...
ggplot(perMIN, aes(x = -dbirds, y = num, group = Yr, fill = Yr)) + 
  ...
  scale_x_continuous(name = "Disturbance magnitude (change in number of birds)", breaks = 4:10, labels = 4:10) + 
  scale_y_continuous(name = "Number of events") + 
  scale_fill_manual(values = met.brewer("Demuth", 3), name = "") + 
  theme_classic() + 
  theme(legend.position = c(0.7, 0.7))

ggsave("ObjectDetectionInference/figures/FigAI_Disturb.jpg", width = 2.3*5, height = 1.8*5, units = "cm")
